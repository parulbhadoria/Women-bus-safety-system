...
export default function LiveMap() {
  ...
  useEffect(() => {
    ...
    const unsubscribeBusLocation = onValue(busRef, (snap) => {
      const d = snap.val();
      if (d?.latitude && d?.longitude) setBus({ lat: d.latitude, lng: d.longitude });
    });
    unsubscribeBusLocationRef.current = unsubscribeBusLocation;
    watchRef.current = navigator.geolocation.watchPosition((p) => setYou({ lat: p.coords.latitude, lng: p.coords.longitude }));
  ...
  useEffect(() => {
    ...
    setDistance(d);
    if (d <= threshold) completeJourney();
  ...
  return (
    <div className="page-wrap" style={{ display: "grid", gap: 12 }}>
      ...
      <button className="danger-btn pulse" onClick={() => navigate("/passenger/sos", { state: { busNumber: state?.busNumber } })}><FaExclamationTriangle /> SOS</button>
    </div>
  );
}
