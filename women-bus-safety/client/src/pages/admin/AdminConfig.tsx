...
export default function AdminConfig() {
  ...
  useEffect(() => {
    getDoc(doc(db, "app_config", "config")).then(async (s) => {
      const loadedThreshold = s.data()?.thresholdDistance || 500;
      setThresholdDistance(loadedThreshold);
      if (loadedThreshold > 100) {
        await updateDoc(doc(db, "app_config", "config"), { thresholdDistance: 50 });
